    ...
    while True:
        try:
            if ser.is_open:
                ...
                print(f"Гіроскоп: gyroX={new_data['gyroX']}, gyroY={new_data['gyroY']}, gyroZ={new_data['gyroZ']}")
...
def update_plot(frame):
    ...
    try:
        ...
        plt.plot(data['time'], data['gyroX'], label='Gyro X', linestyle='dashed')
        plt.plot(data['time'], data['gyroY'], label='Gyro Y', linestyle='dashed')
        plt.plot(data['time'], data['gyroZ'], label='Gyro Z', linestyle='dashed')
